...
with open("input_user_defined_2D.csv") as fp:
	line = fp.readline()
	end_of_tuple=0
	while line:
		if line!='\n' and end_of_tuple==0:
...
with open('output_user_defined_2D.csv', 'w') as f:
	f.writelines(' '.join(str(j) for j in i) + '\t' + ''.join(str(reliabilities[roads.index(i)])) + '\t' +''.join(str(data_loss[roads.index(i)])) + '\n' for i in roads)
with open("routes_user_defined_2D.txt", "wb") as fp:
	pickle.dump(roads, fp)
with open("reliabilities_user_defined_2D.txt", "wb") as fp:
	pickle.dump(reliabilities, fp)
with open("data_loss_user_defined_2D.txt", "wb") as fp:
...
rels5.sort()
###############################################################################################################
with open('ot_user_defined_2D.txt', 'w') as f:
